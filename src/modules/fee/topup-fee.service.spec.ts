...
describe('TopupFeeService', () => {
  let service: TopupFeeService;
  ...
  beforeEach(async () => {
    ...
    service = module.get<TopupFeeService>(TopupFeeService);
  });
  ...
  describe('calculateTopupFee', () => {
    ...
    it('should calculate fees with 3 agent shareholders (subtracting from nominal)', async () => {
      ...
      const result = await service.calculateTopupFee(filter as any);

      // Provider fee: 100 + (10000 * 1/100) = 200
      expect(result.providerFee.nominal.toNumber()).toBe(200);
      expect(result.providerFee.name).toBe('INTERNAL');
      ...
      // Merchant net: 10000 - 200 - 400 - 100 = 9300 (topup subtracts fees)
    ...
    it('should throw when baseFee is not found', async () => {
      ...
      await expect(service.calculateTopupFee(filter as any)).rejects.toThrow(
        'No BaseFee found',
      );
    });
