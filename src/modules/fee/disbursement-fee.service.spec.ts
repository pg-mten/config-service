...
describe('DisbursementFeeService', () => {
  let service: DisbursementFeeService;
  ...
  beforeEach(async () => {
    ...
    service = module.get<DisbursementFeeService>(DisbursementFeeService);
  });
  ...
  describe('calculateDisbursementFee', () => {
    const filter = {
      merchantId: 1,
      providerName: 'PDN',
    ...
    it('should calculate fees with 3 agent shareholders (adding to nominal)', async () => {
      ...
      const result = await service.calculateDisbursementFee(filter as any);

      // Provider fee: 100 + (10000 * 1/100) = 100 + 100 = 200
      ...
      // Internal fee: 200 + (10000 * 2/100) = 200 + 200 = 400
      ...
      // Agent fee total: 50 + (10000 * 0.5/100) = 50 + 50 = 100
      ...
      // Merchant net: 10000 + 200 + 400 + 100 = 10700 (disbursement adds fees)
    ...
    it('should calculate fees without agents when agent fees are zero', async () => {
      ...
      // agentShareholder.findMany should NOT have been called
      expect(
    ...
    it('should throw when baseFee is not found', async () => {
      ...
      await expect(
        service.calculateDisbursementFee(filter as any),
      ).rejects.toThrow('No BaseFee found');
    });
    ...
    it('should throw when merchantFee is not found', async () => {
      ...
      await expect(
        service.calculateDisbursementFee(filter as any),
      ).rejects.toThrow('No MerchantFee found');
    });
  });
});
